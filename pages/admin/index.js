      ...
      {['USD', 'PKR'].map(cur => (
        <button key={cur} onClick={() => setCurrency(cur)}
          style={{
            ...
            boxShadow: currency === cur ? '0 0 16px rgba(0,200,255,0.3)' : 'none',
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    ]).then(([u, m]) => { setUser(u); setMonths(Array.isArray(m) ? m : []); setLoading(false) })
  }, [])

  async function handleStatusChange(id, status) {
    const m = months.find(x => x.id === id)
    if (!m) return
    const res = await fetch(`/api/admin/months/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...m, paymentStatus: status }) })
    if (res.ok) { const u = await res.json(); setMonths(prev => prev.map(x => x.id === id ? u : x)) }
  }
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontFamily: 'Orbitron, monospace', fontSize: '0.72rem', color: c.textMuted, letterSpacing: 1 }}>Loading…</p>
      ...
      <Layout user={user} adminLinks={NAV} investorLinks={NAV}>
        ...
        {months.length === 0 && (
          <div className="card" style={{ textAlign: 'center', padding: '48px 24px' }}>
            ...
            <p style={{ fontFamily: 'Orbitron, monospace', fontSize: '0.9rem', color: c.textPrimary, marginBottom: 6 }}>No data yet</p>
            <p style={{ fontFamily: 'Exo 2, sans-serif', fontSize: '0.85rem', color: c.textMuted, marginBottom: 20 }}>Seed historical data or add a new month to get started</p>
        ...
        {months.length > 0 && (
          <>
            ...
            <div className="card" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 18 }}>
              <div>
                <p style={{ fontFamily: 'Orbitron, monospace', fontSize: '0.6rem', color: c.textMuted, letterSpacing: 1.2, textTransform: 'uppercase', marginBottom: 6 }}>
                  Total Investor Payouts (30%)
                </p>
                <p style={{ fontFamily: 'Orbitron, monospace', fontSize: '1.6rem', fontWeight: 700, color: c.cyan, textShadow: `0 0 20px ${c.cyan}66`, margin: 0 }}>
                  {fmt(totalShare, currency)}
                </p>
                <p style={{ fontFamily: 'Exo 2, sans-serif', fontSize: '0.78rem', color: c.textMuted, marginTop: 5 }}>
                  {filtered.filter(m => m.paymentStatus === 'Received').length} received · {filtered.filter(m => m.paymentStatus === 'Pending').length} pending
                </p>
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                <Link href="/admin/entry" className="btn-filled btn-vm" style={{ fontSize: '0.82rem', padding: '9px 20px' }}>+ New Month</Link>
                <Link href="/admin/investors" className="btn-ghost btn-vm" style={{ fontSize: '0.82rem', padding: '9px 20px' }}>Manage Investors</Link>
              </div>
            </div>
