    ...
    <>
      ...
      <Layout user={user} investorLinks={NAV} adminLinks={NAV}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 14, marginBottom: 18 }}>
          {[
            { label: 'Received', val: totalReceived, color: c.green, bgColor: `${c.green}18`, borderColor: `${c.green}30`, icon: '✓', sub: `${received.length} months paid` },
            { label: 'Pending', val: totalPending, color: c.amber, bgColor: `${c.amber}18`, borderColor: `${c.amber}30`, icon: '⏳', sub: pending.length > 0 ? `${pending.length} month${pending.length > 1 ? 's' : ''} pending` : 'All payments received' },
          ].map(({ label, val, color, bgColor, borderColor, icon, sub }) => (
            <div key={label} className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div style={{ width: 48, height: 48, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, background: bgColor, border: `1px solid ${borderColor}` }}>
                {icon}
              </div>
            </div>
          ))}
